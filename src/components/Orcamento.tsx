...
// ── ComboBox híbrido: digita ou escolhe da lista ──────────────────────────────
  ...
  return (
    <div ref={ref} className="relative">
      <div className={`flex items-center bg-white border rounded-lg transition-all ${
        ...
        />
        {texto && (
          <button type="button" onMouseDown={e => { e.preventDefault(); setTexto(''); onChange(''); setAberto(false); }}
            className="px-1 text-slate-300 hover:text-slate-500">
            <X className="w-3.5 h-3.5" />
          </button>
        )}
        <button type="button" onMouseDown={e => { e.preventDefault(); setAberto(v => !v); }}
          className="pr-2.5 pl-1 text-slate-400 hover:text-blue-600" tabIndex={-1}>
          ...
          <motion.div initial={{ opacity: 0, y: -4, scaleY: 0.95 }} animate={{ opacity: 1, y: 0, scaleY: 1 }}
            ...
            <div className="flex items-center gap-1.5 px-3 py-2 bg-slate-50 border-b border-slate-100">
              ...
              <span className="text-xs text-slate-500">
                {opcoes.length === 0 ? 'Nenhuma diretoria na base' : `${filtradas.length} encontrada(s)`}
              </span>
            </div>
            {filtradas.length > 0 ? (
                ...
                {filtradas.map(op => (
                  <li key={op}>
                    <button type="button" onMouseDown={e => { e.preventDefault(); handleSelect(op); }}
                      ...
                      {op}
                      {op === value && <Check className="w-3.5 h-3.5 text-blue-600" />}
                    </button>
                  </li>
                ))}
              </ul>
              ...
              <div className="px-3 py-3 text-xs text-slate-400 text-center">
                {texto ? `"${texto}" será usado como nova diretoria` : 'Digite um nome de diretoria'}
              </div>
            )}
            {texto && !opcoes.includes(texto) && (
              <div className="border-t border-slate-100">
                <button type="button" onMouseDown={e => { e.preventDefault(); handleSelect(texto); }}
                  ...
                  Usar "<strong>{texto}</strong>" como nova diretoria
                </button>
              </div>
            )}
          </motion.div>
...
export default function Orcamento({
  ...
  // ── Diretorias únicas da base de dados ──
  const diretoriasDaBase = useMemo(
    () => [...new Set(dados.map(d => d.diretoria).filter(Boolean))].sort(),
    [dados]
  );
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-5 gap-3">
            ...
            <div className="lg:col-span-1">
              <label className="text-xs text-slate-500 mb-1 block">
                Diretoria *
                {diretoriasDaBase.length > 0 && (
                  <span className="ml-1 text-blue-500">({diretoriasDaBase.length} na base)</span>
                )}
              </label>
              <ComboBox
                value={novo.diretoria}
                onChange={val => { setNovo(n => ({ ...n, diretoria: val })); setErroNovo(''); }}
                opcoes={diretoriasDaBase}
