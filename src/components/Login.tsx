...
export default function Login() {
  ...
  const handleLogin = async (e: React.FormEvent) => {
    ...
    if (!resultado.ok) {
      setErroLogin(resultado.motivo || 'Erro ao entrar.');
    }
  ...
  const handleCadastro = async (e: React.FormEvent) => {
    ...
    if (!cadNome.trim()) {
      setErroCad('Informe seu nome.');
      return;
    }

    if (!cadLogin.trim()) {
      setErroCad('Informe um nome de usuário.');
      return;
    }

    if (cadSenha.length < 4) {
      setErroCad('A senha deve ter pelo menos 4 caracteres.');
      return;
    }
    ...
    const resultado = await registrar(cadNome.trim(), cadLogin.trim(), cadSenha, cadRole);
  ...
  return (
    <div
      ...
      style={{ background: 'linear-gradient(135deg, #1C2340 0%, #2A3356 50%, #1C2340 100%)' }}
    >
      <motion.div
              ...
              >
                ...
                <div className="mt-4 grid grid-cols-2 gap-2">
                  <div
                    ...
                    style={{ background: '#fdf2f2', borderColor: '#f5c6cb' }}
                  >
                    <Shield
                  ...
                  >
                    <div className="flex items-start gap-3">
                      ...
                      <div>
                        ...
                        >
                          {cadRole === 'operador'
                            ? 'Agora aguarde o administrador liberar seu acesso. Você só conseguirá entrar depois da aprovação.'
                            : sucessoCad}
                        </p>

                        {cadRole === 'operador' && (
                          <p className="text-xs text-amber-600 mt-2">
                            Dica: avise o administrador para acessar a aba "Usuários" e aprovar sua solicitação.
                          </p>
                        )}
                      </div>
                    </div>
                    ...
                    <button
                      ...
                      className={`mt-4 w-full py-2 text-white text-sm font-medium rounded-lg transition-colors ${
                        cadRole === 'operador'
                      ...
                      {cadRole === 'operador' && (
                        <div className="flex items-center gap-1.5 mt-2 px-3 py-2 bg-amber-50 border border-amber-200 rounded-lg">
                          ...
                          <p className="text-xs text-amber-700">
                            Requer aprovação do administrador antes de fazer login
                          </p>
                        </div>
                      )}
                    </div>
                    ...
                    <div>
                      ...
                      <div className="relative">
                        ...
                        <input
                          ...
                          value={cadLogin}
                          onChange={e => {
                            setCadLogin(e.target.value.replace(/\s/g, ''));
                            setErroCad('');
                          }}
