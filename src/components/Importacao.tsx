    ...
    reader.onload = (e) => {
      try {
        ...
        dadosLinhas.forEach((row, i) => {
          if (!row || row.every(c => String(c).trim() === '')) return; // pula linhas vazias
    ...
    <div className="max-w-4xl mx-auto space-y-6">
      ...
      >
        ...
        <AnimatePresence mode="wait">
            ...
            <motion.div key="arquivo" initial={{ opacity: 0 }} animate={{ opacity: 1 }} exit={{ opacity: 0 }} className="space-y-5">
              ...
              <div className="bg-amber-50 border border-amber-200 rounded-xl p-4">
                ...
                <p className="text-sm text-amber-700 mb-3">
                  O arquivo deve ter as colunas nessa ordem (com ou sem cabeçalho):
                </p>
          ...
          {/* ── PRÉ-VISUALIZAÇÃO (compartilhada entre abas) ── */}
          {modo === 'preview' && (
