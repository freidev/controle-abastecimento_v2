...
interface Filtros {
  ccNovo: string; diretoria: string; gerencia: string; areaLot: string;
  fornecedor: string; area: string; semana: string; dataInicio: string; dataFim: string;
}
...
export default function BaseDados({ dados, precoDiesel, onDelete, onClearAll, onEdit }: BaseDadosProps) {
  ...
  const iniciarEdicao = (item: Abastecimento) => { setEditandoId(item.id); setFormEdit({ ...item }); setConfirmDelete(null); };
        ...
        <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-4">
          ...
          <div className="flex items-center gap-2 flex-wrap">
            <div className="relative">
              ...
              <input type="text" placeholder="Buscar em todos os campos..." value={busca}
            ...
            <button onClick={() => setMostrarFiltros(v => !v)}
              className={`flex items-center gap-1.5 px-3 py-2 text-sm font-medium rounded-lg border transition-all ${
                mostrarFiltros || hasFiltros ? 'bg-blue-600 text-white border-blue-600' : 'bg-slate-50 text-slate-600 border-slate-200 hover:border-blue-300 hover:text-blue-700'
              }`}>
              <SlidersHorizontal className="w-4 h-4" />
              Filtros
              {hasFiltros && <span className="px-1.5 py-0.5 bg-white/20 rounded text-xs font-bold">{totalFiltrosAtivos}</span>}
            ...
            <motion.div initial={{ opacity: 0, height: 0 }} animate={{ opacity: 1, height: 'auto' }} exit={{ opacity: 0, height: 0 }} className="overflow-hidden">
              <div className="mt-4 pt-4 border-t border-slate-100">
                <div className="flex items-center justify-between mb-3">
                  ...
                  {hasFiltros && (
                    <button onClick={limparFiltros} className="flex items-center gap-1 text-xs text-red-500 hover:text-red-700 font-medium transition-colors">
                      <X className="w-3 h-3" /> Limpar filtros ({totalFiltrosAtivos})
                    </button>
                  )}
                </div>
                <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 xl:grid-cols-5 gap-3">
                  <div><label className="text-xs font-medium text-slate-500 mb-1 block">CC NOVO</label>
                    <select value={filtros.ccNovo} onChange={setFiltro('ccNovo')} className={selectFiltro}>
                      <option value="">Todos</option>{opcoes.ccNovo.map(v => <option key={v} value={v}>{v}</option>)}
                    </select></div>
                  <div><label className="text-xs font-medium text-slate-500 mb-1 block">Diretoria</label>
                    <select value={filtros.diretoria} onChange={setFiltro('diretoria')} className={selectFiltro}>
                      <option value="">Todas</option>{opcoes.diretoria.map(v => <option key={v} value={v}>{v}</option>)}
                    </select></div>
                  <div><label className="text-xs font-medium text-slate-500 mb-1 block">Gerência</label>
                    <select value={filtros.gerencia} onChange={setFiltro('gerencia')} className={selectFiltro}>
                      <option value="">Todas</option>{opcoes.gerencia.map(v => <option key={v} value={v}>{v}</option>)}
                    </select></div>
                  <div><label className="text-xs font-medium text-slate-500 mb-1 block">Área Lot.</label>
                    <select value={filtros.areaLot} onChange={setFiltro('areaLot')} className={selectFiltro}>
                      <option value="">Todas</option>{opcoes.areaLot.map(v => <option key={v} value={v}>{v}</option>)}
                    </select></div>
                  <div><label className="text-xs font-medium text-slate-500 mb-1 block">Fornecedor</label>
                    <select value={filtros.fornecedor} onChange={setFiltro('fornecedor')} className={selectFiltro}>
                      <option value="">Todos</option>{opcoes.fornecedor.map(v => <option key={v} value={v}>{v}</option>)}
                    </select></div>
                  <div><label className="text-xs font-medium text-slate-500 mb-1 block">Área</label>
                    <select value={filtros.area} onChange={setFiltro('area')} className={selectFiltro}>
                      <option value="">Todas</option>{opcoes.area.map(v => <option key={v} value={v}>{v}</option>)}
                    </select></div>
                  <div><label className="text-xs font-medium text-slate-500 mb-1 block">Semana</label>
                    <select value={filtros.semana} onChange={setFiltro('semana')} className={selectFiltro}>
                      <option value="">Todas</option>{opcoes.semana.map(v => <option key={v} value={v}>Semana {v}</option>)}
                    </select></div>
                  <div><label className="text-xs font-medium text-slate-500 mb-1 block">Data Início</label>
                    <input type="date" value={filtros.dataInicio} onChange={setFiltro('dataInicio')} className={selectFiltro} /></div>
                  <div><label className="text-xs font-medium text-slate-500 mb-1 block">Data Fim</label>
                    <input type="date" value={filtros.dataFim} onChange={setFiltro('dataFim')} className={selectFiltro} /></div>
                </div>
                {hasFiltros && (
                  <div className="flex flex-wrap gap-1.5 mt-3 pt-3 border-t border-slate-100">
                    {Object.entries(filtros).map(([key, val]) => {
                      if (!val) return null;
                      const labels: Record<string, string> = { ccNovo: 'CC NOVO', diretoria: 'Diretoria', gerencia: 'Gerência', areaLot: 'Área Lot.', fornecedor: 'Fornecedor', area: 'Área', semana: 'Semana', dataInicio: 'De', dataFim: 'Até' };
                      const display = key === 'semana' ? `Sem. ${val}` : key.startsWith('data') ? new Date(val).toLocaleDateString('pt-BR') : val;
                      return (
                        <span key={key} className="inline-flex items-center gap-1.5 px-2.5 py-1 bg-blue-50 border border-blue-200 text-blue-700 text-xs rounded-full">
                          ...
                          <button onClick={() => { setFiltros(f => ({ ...f, [key]: '' })); setPagina(1); }} className="text-blue-400 hover:text-blue-700 transition-colors"><X className="w-3 h-3" /></button>
                        </span>
                      );
                    })}
                  </div>
      ...
      <motion.div initial={{ opacity: 0 }} animate={{ opacity: 1 }} transition={{ delay: 0.1 }}
            ...
            <tbody className="divide-y divide-slate-100">
                    ...
                    <motion.tr key={item.id} initial={{ opacity: 0, x: -10 }} animate={{ opacity: 1, x: 0 }}
                      ...
                      <td className="px-3 py-2.5 text-center">
                        <div className="flex items-center justify-center gap-1">
                          <button onClick={() => iniciarEdicao(item)} className="p-1.5 text-blue-400 hover:text-blue-600 hover:bg-blue-50 rounded transition-colors" title="Editar">
                            <Edit3 className="w-4 h-4" />
                          </button>
                          {confirmDelete === item.id ? (
                            <div className="flex items-center gap-1">
                              <button onClick={() => { onDelete(item.id); setConfirmDelete(null); }} className="p-1 bg-red-100 text-red-600 rounded hover:bg-red-200 transition-colors"><AlertTriangle className="w-3.5 h-3.5" /></button>
                              <button onClick={() => setConfirmDelete(null)} className="p-1 bg-slate-100 text-slate-600 rounded hover:bg-slate-200 transition-colors"><X className="w-3.5 h-3.5" /></button>
                            </div>
                          ) : (
                            <button onClick={() => setConfirmDelete(item.id)} className="p-1.5 text-slate-400 hover:text-red-500 hover:bg-red-50 rounded transition-colors"><Trash2 className="w-4 h-4" /></button>
                          )}
                        </div>
                      </td>
                    </motion.tr>
                        ...
                        <td colSpan={13} className="px-4 py-4">
                          <motion.div initial={{ opacity: 0, y: -6 }} animate={{ opacity: 1, y: 0 }} className="bg-white border border-blue-200 rounded-xl p-4 shadow-sm">
                            ...
                            <div className="grid grid-cols-2 sm:grid-cols-3 lg:grid-cols-4 gap-3">
                              <div><label className="text-xs text-slate-500 mb-1 block">CC Novo</label><input type="text" value={formEdit.ccNovo || ''} onChange={e => setFormEdit(f => ({ ...f, ccNovo: e.target.value }))} className={inputCls} /></div>
                              <div><label className="text-xs text-slate-500 mb-1 block">Diretoria</label>
                                <select value={formEdit.diretoria || ''} onChange={e => setFormEdit(f => ({ ...f, diretoria: e.target.value }))} className={selectCls}>
                                  <option value="">Selecione...</option>{DIRETORIAS.map(d => <option key={d} value={d}>{d}</option>)}
                                </select></div>
                              <div><label className="text-xs text-slate-500 mb-1 block">Gerência</label><input type="text" value={formEdit.gerencia || ''} onChange={e => setFormEdit(f => ({ ...f, gerencia: e.target.value }))} className={inputCls} /></div>
                              <div><label className="text-xs text-slate-500 mb-1 block">Área Lotação</label>
                                <select value={formEdit.areaLot || ''} onChange={e => setFormEdit(f => ({ ...f, areaLot: e.target.value }))} className={selectCls}>
                                  <option value="">Selecione...</option>{AREAS_LOT.map(a => <option key={a} value={a}>{a}</option>)}
                                </select></div>
                              <div><label className="text-xs text-slate-500 mb-1 block">Fornecedor</label>
                                <select value={formEdit.fornecedor || ''} onChange={e => setFormEdit(f => ({ ...f, fornecedor: e.target.value }))} className={selectCls}>
                                  <option value="">Selecione...</option>{FORNECEDORES.map(f => <option key={f} value={f}>{f}</option>)}
                                </select></div>
                              <div><label className="text-xs text-slate-500 mb-1 block">Equipamento</label>
                                <select value={formEdit.equipamento || ''} onChange={e => setFormEdit(f => ({ ...f, equipamento: e.target.value }))} className={selectCls}>
                                  <option value="">Selecione...</option>{EQUIPAMENTOS.map(eq => <option key={eq} value={eq}>{eq}</option>)}
                                </select></div>
                              <div><label className="text-xs text-slate-500 mb-1 block">Área</label>
                                <select value={formEdit.area || ''} onChange={e => setFormEdit(f => ({ ...f, area: e.target.value }))} className={selectCls}>
                                  <option value="">Selecione...</option>{AREAS.map(a => <option key={a} value={a}>{a}</option>)}
                                </select></div>
                              <div><label className="text-xs text-slate-500 mb-1 block">Data</label><input type="date" value={formEdit.data || ''} onChange={e => setFormEdit(f => ({ ...f, data: e.target.value }))} className={inputCls} /></div>
                              <div><label className="text-xs text-slate-500 mb-1 block">Litros</label><input type="number" step="0.01" value={formEdit.litros || ''} onChange={e => setFormEdit(f => ({ ...f, litros: Number(e.target.value) }))} className={inputCls} /></div>
                            </div>
                            <div className="flex gap-2 mt-4 pt-3 border-t border-blue-100">
                              <button onClick={salvarEdicao} className="flex items-center gap-2 px-5 py-2 bg-blue-600 hover:bg-blue-700 text-white text-sm font-semibold rounded-lg transition-colors shadow-sm"><Save className="w-4 h-4" /> Salvar Alterações</button>
                              <button onClick={cancelarEdicao} className="flex items-center gap-2 px-4 py-2 bg-slate-100 hover:bg-slate-200 text-slate-700 text-sm font-medium rounded-lg transition-colors"><X className="w-4 h-4" /> Cancelar</button>
                            </div>
                          </motion.div>
                        </td>
                      </tr>
              ...
              {dadosPaginados.length === 0 && (
                <tr><td colSpan={13} className="px-3 py-12 text-center text-slate-400">
                  {hasFiltros ? (
                    <div className="space-y-2"><p>Nenhum registro encontrado com os filtros aplicados.</p>
                      <button onClick={limparFiltros} className="text-blue-600 hover:text-blue-700 text-sm font-medium">Limpar filtros</button>
                    </div>
        ...
        {totalPaginas > 1 && (
          <div className="flex items-center justify-between px-5 py-3 border-t border-slate-200 bg-slate-50">
            ...
            <div className="flex items-center gap-1">
              <button onClick={() => setPagina(p => Math.max(1, p - 1))} disabled={pagina === 1} className="p-1.5 rounded-lg hover:bg-slate-200 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"><ChevronLeft className="w-4 h-4" /></button>
              {Array.from({ length: Math.min(totalPaginas, 7) }, (_, i) => {
                const p = totalPaginas <= 7 ? i + 1 : pagina <= 4 ? i + 1 : pagina >= totalPaginas - 3 ? totalPaginas - 6 + i : pagina - 3 + i;
                return <button key={p} onClick={() => setPagina(p)} className={`w-8 h-8 rounded-lg text-sm font-medium transition-colors ${p === pagina ? 'bg-blue-600 text-white' : 'hover:bg-slate-200 text-slate-600'}`}>{p}</button>;
              })}
              <button onClick={() => setPagina(p => Math.min(totalPaginas, p + 1))} disabled={pagina === totalPaginas} className="p-1.5 rounded-lg hover:bg-slate-200 disabled:opacity-40 disabled:cursor-not-allowed transition-colors"><ChevronRight className="w-4 h-4" /></button>
            </div>
          </div>
        )}
      </motion.div>
