...
interface CCNovoInputProps {
  ...
  labelVazio?: string;   // mensagem quando não há sugestões
  labelDigite?: string;  // mensagem "Digite ou selecione abaixo"
}
          ...
          >
            ...
            <div className="flex items-center gap-1.5 px-3 py-2 border-b border-slate-100 bg-slate-50">
              ...
              <span className="text-xs text-slate-500">
                {sugestoes.length === 0
                  ? labelVazio
                  : `${opcoesFiltradas.length} encontrado${opcoesFiltradas.length !== 1 ? 's' : ''}`
                }
              </span>
            </div>
            ...
            {opcoesFiltradas.length > 0 ? (
              ...
              <div className="px-3 py-3 text-xs text-slate-400 text-center">
                {filtro
                  ? <>Nenhum resultado para "<strong>{filtro}</strong>" — será usado como novo</>
                  : labelDigite
                }
              </div>
            )}
...
interface EditorRateioProps {
  ...
  // mapa de gerência → lista de CCs vinculados
...
function EditorRateio({ rateio, onSave, onCancel, equipamentosDisponiveis, ccsCadastrados, gerenciasDaBase, ccsPorGerencia }: EditorRateioProps) {
  ...
  const addParcela = () => {
    ...
    setForm(f => ({
      ...f,
      parcelas: [...f.parcelas, { ccNovo: '', descricaoCC: '', percentual: restante, gerencia: '' }],
    }));
  };
  ...
  const updateParcela = (i: number, field: keyof RateioParcela, value: string | number) => {
    setForm(f => {
      ...
      // Se mudou a gerência, limpa o CC para forçar nova seleção
  ...
  const handleSave = () => {
    if (!validar()) return;
    // Salva a gerência principal como a primeira gerência das parcelas
  ...
  return (
    <div className="space-y-5">
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        <div>
          ...
          <CCNovoInput
            ...
            onChange={val => setForm(f => ({ ...f, equipamento: val }))}
            sugestoes={equipamentosDisponiveis}
            placeholder="Selecione ou digite o equipamento..."
          />
          {equipamentosDisponiveis.length === 0 && (
            <p className="text-xs text-amber-600 mt-1">⚠️ Nenhum equipamento na base ainda</p>
          )}
        </div>
        <div>
          <label className="text-xs font-semibold text-slate-600 mb-1.5 block uppercase tracking-wide">
      ...
      {/* Parcelas — cada uma com Gerência + CC NOVO */}
      <div>
        <div className="flex items-center justify-between mb-3">
          <div>
            <label className="text-xs font-semibold text-slate-600 uppercase tracking-wide">
              Gerências, Centros de Custo e Porcentagens
            </label>
        ...
        <div className="space-y-3">
          {form.parcelas.map((parcela, i) => {
            // CCs filtrados pela gerência desta parcela
            const ccsDaGerencia = parcela.gerencia
              ? (ccsPorGerencia[parcela.gerencia] || [])
              : ccsCadastrados;
            ...
            return (
              <motion.div key={i} initial={{ opacity: 0, y: -8 }} animate={{ opacity: 1, y: 0 }}
                ...
                {/* Linha superior: número + gerência + botão remover */}
                <div className="flex items-center gap-2">
                  <span className="w-6 h-6 rounded-full bg-blue-100 text-blue-700 text-xs font-bold flex items-center justify-center flex-shrink-0">
                    {i + 1}
                  </span>
                  <div className="flex-1">
                    <label className="text-xs text-slate-500 mb-0.5 block font-medium">
                      Gerência <span className="text-red-400">*</span>
                    </label>
                    <CCNovoInput
                      value={parcela.gerencia || ''}
                      onChange={val => updateParcela(i, 'gerencia', val)}
                      sugestoes={gerenciasDaBase}
                      placeholder="Selecione ou digite a gerência..."
                      labelVazio="Nenhuma Gerência cadastrada"
                      labelDigite="Digite uma Gerência ou selecione abaixo"
                    />
                  </div>
                ...
                {/* Linha inferior: CC NOVO + Descrição + % */}
                <div className="grid grid-cols-12 gap-2 pl-8">
                  <div className="col-span-3">
                    <label className="text-xs text-slate-400 mb-0.5 block">
                      ...
                      {parcela.gerencia && ccsDaGerencia.length > 0 && (
                        <span className="ml-1 text-blue-500">({ccsDaGerencia.length} disponível/is)</span>
                      )}
                    </label>
                    <CCNovoInput
                      value={parcela.ccNovo}
                      onChange={val => updateParcela(i, 'ccNovo', val)}
                      sugestoes={ccsDaGerencia}
                      blockedValues={ccsBloqueados}
                  ...
                  <div className="col-span-6">
                    ...
                    <input type="text" value={parcela.descricaoCC}
        ...
        <div className="mt-3 p-3 rounded-xl border bg-white">
          ...
          <div className="w-full bg-slate-100 rounded-full h-2 overflow-hidden">
            <motion.div animate={{ width: `${Math.min(soma, 100)}%` }}
...
export default function Rateio({ dados, rateios, precoDiesel, onSave }: RateioProps) {
  ...
  // Equipamentos únicos da base de dados — só mostra o que já foi cadastrado
  const equipamentosUsados = rateios.map(r => r.equipamento);
  const equipamentosDaBase = useMemo(
    () => [...new Set(dados.map(d => d.equipamento).filter(Boolean))].sort(),
    [dados]
  );
  // Para novo rateio: remove os que já têm rateio ativo; para edição: mostra todos
  const equipamentosDisponiveis = useMemo(
    () => modoEdicao === 'novo'
      ? equipamentosDaBase.filter(eq => !equipamentosUsados.includes(eq))
      : equipamentosDaBase,
  ...
  // Gerências únicas da base de dados — alimenta o dropdown de gerência
  const gerenciasDaBase = useMemo(
    () => [...new Set(dados.map(d => d.gerencia).filter(Boolean))].sort(),
    [dados]
  );

  // Mapa: gerência → lista de CCs únicos vinculados a ela na base
              ...
              <tbody className="divide-y divide-slate-100">
                {rateios.filter(r => r.ativo).flatMap(r => {
                  ...
                  return r.parcelas.map((p, i) => ({
                    equipamento: r.equipamento,           // ← sempre mostra
                    gerencia:    p.gerencia || r.gerencia || '—', // ← gerência da parcela
                    isFirst: i === 0,
                    cc:   p.ccNovo,
                    desc: p.descricaoCC,
                    perc: p.percentual,
                  ...
                  <tr key={idx} className="hover:bg-slate-50">
                    ...
                    {/* Gerência da parcela — sempre visível */}
                    <td className="px-3 py-2.5 text-xs">
                      {row.gerencia && row.gerencia !== '—' ? (
                        <span className="px-2 py-0.5 bg-blue-50 text-blue-700 rounded-full font-medium text-xs">
                          {row.gerencia}
                        </span>
                      ) : (
